    ...
    conexion.query(sqlCotizacion, [id], async (err, rowsCot) => {
        ...
        conexion.query(sqlPiezas, [id], (err, rowsPiezas) => {
            ...
            conexion.query(sqlAccesorios, [id], async (err, rowsAccesorios) => {
                ...
                if (!finalPdfBuffer) {
                    console.log(`[MAIL] Generando PDF unificado en servidor para cotización #${cotizacion.id_cotizacion}...`);
                    try {
                        const { generarPDFCotizacionServer } = require('../utils/pdfGenerator');
                        finalPdfBuffer = generarPDFCotizacionServer({
                            cotizacion,
                            piezas: rowsPiezas,
                            accesorios: rowsAccesorios
                        });
